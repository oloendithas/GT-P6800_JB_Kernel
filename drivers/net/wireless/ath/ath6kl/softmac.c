...
static int ath6kl_fetch_nvmac_info(struct ath6kl *ar)
{
	...
	do {
		...
		if (!strncmp(ath6kl_softmac, "12:34:56", 8)) {
			strncpy(ath6kl_softmac, "00:12:34", 8);
			android_readwrite_file(softmac_filename,
					NULL,
					ath6kl_softmac, 17);
			ath6kl_dbg(ATH6KL_DBG_BOOT,
                                        "%s: Convert mac prefix.\n",
                                        __func__);
		}
