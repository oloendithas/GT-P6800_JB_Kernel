				    ...
				    u8 *assoc_info)
{
	...
	if (vif->nw_type == AP_NETWORK) {
		...
		if (memcmp(dev->dev_addr, u->ap_bss.bssid, ETH_ALEN) == 0) {
			...
			ath6kl_dbg(ATH6KL_DBG_WMI, "%s: aid %u mac_addr %s "
				   ...
				   sec_conv_mac(u->ap_sta.mac_addr),
				   u->ap_sta.auth,
				   u->ap_sta.keymgmt,
	...
	ath6kl_dbg(ATH6KL_DBG_WMI,
		   ...
		   le16_to_cpu(u->sta.ch), sec_conv_mac(u->sta.bssid),
					  ...
					  struct ath6kl_vif *vif)
{
	...
	ath6kl_dbg(ATH6KL_DBG_WMI,
		   ...
		   le16_to_cpu(ev->proto_reason_status), sec_conv_mac(ev->bssid),
					       ...
					       int len, struct ath6kl_vif *vif)
{
	...
	for (i = 0; i < ev->num_neighbors; i++) {
		ath6kl_dbg(ATH6KL_DBG_WMI, "neighbor %d/%d - %s 0x%x\n",
			   i + 1, ev->num_neighbors, sec_conv_mac(ev->neighbor[i].bssid),
					  ...
					  struct ath6kl_vif *vif)
{
	...
	ath6kl_dbg(ATH6KL_DBG_WMI, "TXE notify event: peer %s rate %d pkts %d\n",
		   sec_conv_mac(vif->bssid), rate, pkts);
...
static int ath6kl_wmi_aplist_event_rx(struct wmi *wmi, u8 *datap, int len)
{
	...
	for (index = 0; index < ev->num_ap; index++) {
		ath6kl_dbg(ATH6KL_DBG_WMI, "AP#%d BSSID %s Channel %d\n",
			   index, sec_conv_mac(ap_info_v1->bssid), ap_info_v1->channel);
		ap_info_v1++;
	}
			    ...
			    enum nl80211_acl_policy_attr acl_policy, bool reset)
{
	...
	ath6kl_dbg(ATH6KL_DBG_WMI, "set acl mac: index:%d action:%d mac:%s reset:%d\n",
		   cmd->index, cmd->action, sec_conv_mac(cmd->mac), reset);
