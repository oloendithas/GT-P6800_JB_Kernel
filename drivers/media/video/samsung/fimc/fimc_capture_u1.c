		...
		if (ctrl->id != FIMC2) {
			...
			if (ret < 0) {
				...
				if (get_fimc_dev()->active_camera == 1) {
					int sensorRatio = ctrl->cam->width*100/ctrl->cam->height;
					int previewRatio = ctrl->cap->fmt.width*100/ctrl->cap->fmt.height;
					if (sensorRatio != previewRatio) {
						if (previewRatio == 122) {
							ctrl->cam->window.width = ctrl->cam->height*previewRatio/100;
							if (ctrl->cam->window.width%2)
								ctrl->cam->window.width += 1;
							ctrl->cam->window.height = ctrl->cam->height;
							ctrl->cam->window.left = (ctrl->cam->width-ctrl->cam->window.width)>>1;
							ctrl->cam->window.top = 0;
						} else {
							dev_err(ctrl->dev, "not supported ratio\n");
						}
					} else {
