...
u8 mhl_onoff_ex(bool onoff)
{
	...
	if (onoff) {
		...
		sii9234_cancel_callback(sii9234);
...
static void sii9234_detection_callback_worker(struct work_struct *p)
{
	...
	pr_debug("%s()\n", __func__);
	sii9234_detection_callback(sii9234);
	return;
}
...
static int sii9234_detection_callback(struct sii9234_data *sii9234)
{
	int ret;
	u8 value;
	int handled = 0;
...
static void sii9234_cancel_callback(struct sii9234_data *sii9234)
{
	sii9234_mutex_lock(&sii9234->lock);
...
static int sii9234_30pin_callback(struct sii9234_data *sii9234)
{
	int ret;
...
static int sii9234_30pin_cancel_callback(struct sii9234_data *sii9234)
{
       pr_info("sii9234: power off for 9290\n");
       sii9234_mutex_lock(&sii9234->lock);
       sii9234->state = STATE_DISCONNECTED;
       sii9234->pdata->hw_onoff(0);
#ifdef	 CONFIG_SAMSUNG_WORKAROUND_HPD_GLANCE
...
static int mhl_callback(struct notifier_block *this,
	       	       	       	         unsigned long event, void *ptr)
{
	...
	pr_info("sii9234: mhl callback: 0x%X <== 0x%X\n",
					(int)event, sii9234->acc_con_state);
	if ((event&0xF) == (sii9234->acc_con_state&0xF))
		return -EINVAL;

	sii9234->acc_con_state = (int)event;

	switch(event) {
	case MHL_1X_ATTACH:
		/*ret = sii9234_detection_callback(sii9234);*/
		mhl_onoff_ex(1);
		break;
	case MHL_1X_DETACH:
		/*sii9234_cancel_callback(sii9234);*/
...
#ifdef CONFIG_SAMSUNG_MHL_9290
	case MHL_3X_ATTACH:
		ret = sii9234_30pin_callback(sii9234);
		break;
	case MHL_3X_DETACH:
		ret = sii9234_30pin_cancel_callback(sii9234);
		break;
#endif
	default:
		pr_err("sii9234: %s() invalid event\n", __func__);
	}
					      ...
					      const struct i2c_device_id *id)
{
	...
	sii9234->acc_con_nb.notifier_call = mhl_callback;
	acc_register_notifier(&sii9234->acc_con_nb);
	sii9234->acc_con_state = 0;
