...
extern bool check_running_suspend;
#endif
#endif
...
#ifdef CONFIG_MDM_HSIC_PM
#ifdef CONFIG_HSIC_EURONLY_APPLY
        if (!check_running_suspend) {
                //printk(KERN_DEBUG "%s: check_running_suspend is false\n", __func__);
                request_active_lock_set(hsic_pm_dev);
        }
        else {
                printk(KERN_DEBUG "%s: check_running_suspend is true\n", __func__);
                return 0;
        }
#else
        request_active_lock_set(hsic_pm_dev);
