					...
					u16 portstatus, u16 portchange)
{
...
#if defined(CONFIG_USB_SUSPEND)    // LSI patch : hub disconnect by hub thread in LPA
	...
	/* Disconnect any existing devices under this port */
        if (udev) {
                pr_info("%s: %d: portstatus=0x%x, udev->state=0x%x, status=%d, rpm=%d, udev->persist_enabled=%d\n"
                        , __func__, __LINE__, portstatus, udev->state, status, udev->dev.power.runtime_status, udev->persist_enabled);
